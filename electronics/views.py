...
class IndexView(generic.ListView):
    template_name ='category/index1d.html'
...
class ItemDelete(DeleteView):
    model=Item
    success_url= reverse_lazy('electronics:index1d')

def adconf(request,pk):
    ad=Item.objects.get(pk=pk)
    context={'item':ad,}
    print("This is working .")
    return render(request, 'electronics/adconf.html',context)
...
def ItemListsearchView(request):
    ...
    return render(request, 'category/search_result1d.html', context)
