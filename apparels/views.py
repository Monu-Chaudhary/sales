...
class IndexView(generic.ListView):
    template_name ='category/index1e.html'
...
def resultview(request, pk):
    ...
    return render(request, 'apparels/detail.html',context)



class DetailView(generic.DetailView):
    model=Item
    template_name='apparels/detail.html'
...
class ItemDelete(DeleteView):
    model=Item
    print('iam working starting our website welcome iam learning machine learning')
    success_url= reverse_lazy('apparels:index1e')

def adconf(request,pk):
    ad=Item.objects.get(pk=pk)
    context={'item':ad,}
    return render(request, 'apparels/adconf.html',context)
...
def ItemListsearchView(request):
    ...
    return render(request, 'category/search_result1e.html', context)
