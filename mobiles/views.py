...
class IndexView(generic.ListView):
    template_name ='category/index1c.html'
...
def resultview(request, pk):
    ...
    return render(request, 'mobiles/detail.html',context)
...
class ItemDelete(DeleteView):
    model=Item
    success_url= reverse_lazy('mobiles:index1c')

def adconf(request,pk):
    ad=Item.objects.get(pk=pk)
    context={'item':ad,}
    print("This is working .")
    return render(request, 'mobiles/adconf.html',context)
...
def ItemListsearchView(request):
    ...
    return render(request, 'category/search_result1c.html', context)
